...
const { csvReadProxy, csvRegisterSneakerPolitics } = require('../../../utils/csvReader');
...
const { DEV, siteKey, moduleK } = require('../sneakerPoliticsConst');
...
async function createAccount(proxyConfig, user) {
    ...
    try {
        response = await axios({
            ...
            timeout: 6000,
            method: 'POST',
            url: `https://sneakerpolitics.com/account`,
            data: qs.stringify({
        ...
        try {
            if (err.response.data.includes('sneakerpolitics.com/challenge')) {
                let rCookie = err
                console.log(rCookie)
            ...
            if (err.response.data.includes('sneakerpolitics.com/account/register')) {
            ...
            if (err.response.data.includes('sneakerpolitics.com/"')) {
...
async function createAccountAfterCaptcha(proxyConfig, user, sessionId, solvedCaptcha, authenticityToken,rCookie) {
    // process.env["NODE_TLS_REJECT_UNAUTHORIZED"] = 0;
    // console.log(proxyConfig)
    proxyConfig = {
        host: '127.0.0.1',
    ...
    try {
        ...
        */
        ...
        else if (err.response.data.includes('sneakerpolitics.com/challenge')) return { code: 'CHALLENGE', data: undefined };

        // il y a une redirection /register (compte existe déjà)
        else if (err.response.data.includes('sneakerpolitics.com/account/register')) {
...
const update = async (proxyConfig, user) => {
    ...
    try {
        await request({
            ...
            timeout: 3500,
            method: 'POST',
            uri: `https://www.sneakerpolitics.com/account/addresses`,
            body: qs.stringify({
        ...
        fs.appendFileSync('./SneakerPolitics/createdAccount.csv', `\n${user.Email},${user.Password}`);
    } catch (err) {
        if (DEV) console.log(err);
...
async function register() {
    ...
    async function handleProxies(proxies) {
        ...
        await new Promise(async function (resolve) {
            registerData = await csvRegisterSneakerPolitics();
            resolve();
        });
...
async function registerUser(user, proxies, twoCaptchaEnabled) {
    ...
    user = await checkSneakerPoliticsCSV(user);
        ...
        switch (result.code) {
            ...
            case 'CHALLENGE':
                ...
                if (twoCaptchaEnabled) {
                    ...
                    return await solveReCaptcha(siteKey, 'https://sneakerpolitics.com/challenge', onCaptchaSolved);
            ...
            case 'ACCOUNT':
                ...
                fs.appendFileSync('./SneakerPolitics/failedAccount.csv', `\n${user.Email},${user.Password}`);
                return 'ERROR';
            case 'RETRY':
            ...
            case 'CHALLENGE_TOO_LONG':
                ...
                return await solveReCaptcha(siteKey, 'https://www.sneakerpolitics.com/challenge', onCaptchaSolved);
                async function onCaptchaSolved(solvedCaptcha) {
                    logInfo(`[${user.Index}][${user.Email}]` + " | Challenge solved.", true);
                    result = await createAccountAfterCaptcha(proxyConfig, user, result.data.ssid, solvedCaptcha, result.data.authenticity_token);
                    return await handleCreationResult(result);
...
async function checkSneakerPoliticsCSV(registerData) {
